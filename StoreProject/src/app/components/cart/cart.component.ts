...
import { productCart, Product} from '../interface/product-cart';
...
export class CartComponent implements OnInit {
  ...
  onSubmit(x:any) {
    if(this.totalPrice == 0){
      return alert("Por favor agregue un producto al carrito primero")
    }
  ...
  deletaItem(item:productCart){
    ...
    // }) 
    alert(`Se eliminara ${item.product.name}`)
  }
  validateName(x:any){
    console.log(x)
  }
